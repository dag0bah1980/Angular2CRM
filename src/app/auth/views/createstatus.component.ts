...
@Component({
  selector: 'ang2-crm-createstatus',
...
export class CreatestatusComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute,
    private _statusesService: StatusesService, private _cookieService: CookieService,
  ...
  ngOnInit() {
    ...
    this.dataForm = this._fb.group({
      ...
      'Status': [null, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(32), Validators.pattern(CodeValidationRegex)])],
      ...
      'OrderValue': [999, Validators.required],
  ...
  onSubmit() {
    ...
    this.createdStatus = new Status(0, '', '', false, false, '', '','',999);

    this.createdStatus.ISACTIVE = this.dataForm.controls['IsActive'].value;
    this.createdStatus.ISDELETED = this.dataForm.controls['IsDeleted'].value;
    this.createdStatus.STATUSCODE = this.dataForm.controls['StatusCode'].value;
    this.createdStatus.STATUSNAME = this.dataForm.controls['StatusName'].value;    
    this.createdStatus.DESCRIPTION = this.dataForm.controls['Description'].value;
    this.createdStatus.ORDERVALUE = this.dataForm.controls['OrderValue'].value;

    var StatusValue;
    this.StatusValue = this.createdStatus.STATUSCODE;

    //console.log(this.createdTag);
    //console.log(this.statusMessage + ':' + this.statusMessageEmpty);
    this._statusesService.createStatus(this.createdStatus)
      .subscribe(result => {
        if (result == true) {
